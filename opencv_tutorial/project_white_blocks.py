...
cap = cv2.VideoCapture(0) # open and reads the camera

lower_white = np.array([0, 0, 200]) # the values are taken from the chatgpt for the white color range
upper_white = np.array([180, 30, 255])

while True:
    ret, frame = cap.read() # reads the frame in while loop
    if not ret: # checks if the frame is read
